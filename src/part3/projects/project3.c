//-----------------------------------------------------------------------------
//
// Task: Books are identified by an International Standard Book Number (ISBN).
// ISBNs assigned after January 1, 2007 contain 13 digits, arranged in five
// groups, such as 978-0-393-97950-3. (Older ISBNs use 10 digits.)
...
// publisher to identify a specific book (97950 is the code for this book).
// An ISBN ends with a check digit that's used to verify the accuracy of
// the preceding digits. Write a program that breaks down an lSBN entered
// by the user:
//
...
// actual lSBN values (usually found on the back cover of a book and on
// the copyright page).
//
// Date: 2025-1-13
// Time: 9:31 AM
...
int main(void)
{
	int gs1, groupID, pCode, iNumber, check = 0;

	printf("Enter ISBN: ");
	scanf("%d-%d-%d-%d-%d", &gs1, &groupID, &pCode, &iNumber, &check);
